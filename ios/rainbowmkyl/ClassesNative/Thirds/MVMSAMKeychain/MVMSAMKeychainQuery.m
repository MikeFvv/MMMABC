...
@implementation MVMSAMKeychainQuery
...
#ifdef MVMSAMKEYCHAIN_ACCESS_GROUP_AVAILABLE
@synthesize accessGroup = _accessGroup;
#endif

#ifdef MVMSAMKEYCHAIN_SYNCHRONIZATION_AVAILABLE
...
- (BOOL)save:(NSError *__autoreleasing *)error {
	OSStatus status = MVMSAMKeychainErrorBadArguments;
	...
	if (status == errSecSuccess) {//item already exists, update it!
		...
		CFTypeRef accessibilityType = [MVMSAMKeychain accessibilityType];
...
- (NSMutableDictionary *)query {
	...
	if ([[self class] isSynchronizationAvailable]) {
		...
		switch (self.synchronizationMode) {
			case MVMSAMKeychainQuerySynchronizationModeNo: {
			  value = @NO;
			  break;
			}
			case MVMSAMKeychainQuerySynchronizationModeYes: {
			  value = @YES;
			  break;
			}
			case MVMSAMKeychainQuerySynchronizationModeAny: {
...
+ (NSError *)errorWithCode:(OSStatus) code {
	...
	dispatch_once(&onceToken, ^{
		NSURL *url = [[NSBundle bundleForClass:[MVMSAMKeychainQuery class]] URLForResource:@"SAMKeychain" withExtension:@"bundle"];
	...
	switch (code) {
		...
		case MVMSAMKeychainErrorBadArguments: message = NSLocalizedStringFromTableInBundle(@"MVMSAMKeychainErrorBadArguments", @"SAMKeychain", resourcesBundle, nil); break;
	...
	return [NSError errorWithDomain:kMVMSAMKeychainErrorDomain code:code userInfo:userInfo];
}
