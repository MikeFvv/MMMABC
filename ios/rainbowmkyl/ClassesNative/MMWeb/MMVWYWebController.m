...
#import "MMVWYWebProgressLayer.h"
#import "MMVWLWebProgressLayer.h"
...
@interface MMVWYWebController ()<UIWebViewDelegate>
...
//
@property (nonatomic, strong) MMVBoxInfoModel *model;
...
@implementation MMVWYWebController
{
  UIWebView *_webView;
  
  MMVWYWebProgressLayer *_progressLayer; ///< 网页加载进度条
}
...
- (void)webViewDidStartLoad:(UIWebView *)webView {
  ...
  //    _progressLayer = [MMVWYWebProgressLayer layerWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, 2)];
  _progressLayer = [MMVWYWebProgressLayer layerWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 2)];
...
- (void)requestBoxData {
  ...
  [MMNetWorkManager requestWithType:HttpRequestTypeGet withUrlString:phoneApiUrl withParaments:nil withSuccessBlock:^(NSDictionary *object) {
    ...
    if (msg == 0) {
      MMVBoxInfoModels *modelsa = [MMVBoxInfoModels friendWithDict:object];
